...
- Optional risk-based historical guardrails that solve a fixed-spending failsafe, initial spend, lower portfolio cut trigger, and upper portfolio raise trigger from the selected historical backtest set
...
The results Decision panel compares the base plan against ranked rescue choices: cut flexible spending during early market stress, switch to risk-based historical guardrails when historical sequences are available, earn bridge income for a limited number of years, or combine compatible levers. Monte Carlo and historical backtests are shown side by side. If one evidence source passes and the other misses the target, the verdict stays fragile instead of hiding the disagreement in one blended score.
...
The active tax-law year is 2026. Federal brackets, standard deductions, itemized-deduction limits, AMT tripwire thresholds, long-term capital gains thresholds, Child Tax Credit / Additional Child Tax Credit amounts, NIIT, W-2 employee FICA, Additional Medicare Tax, self-employment tax, ACA applicable percentages, and HHS poverty guidelines are versioned in `src/data/taxData.mjs`. State income tax defaults use the generated 2026 state table in `src/data/stateTax2026.generated.mjs`, with manual overrides available in the UI for power users or state-specific nuance not yet modeled.

ACA has two plan-cost modes. State benchmark estimate mode uses the ZIP field, when present, to resolve an offline 2026 SLCSP for **48 bundled states** — the 30 federal-platform states from the CMS FFM PUFs plus 18 State-Based Marketplaces from the CMS SBM QHP PUFs — at **county-level precision** where a county's own silver plan set (CMS Service Area PUFs) yields a different benchmark than its rating area — and at **ZIP-level precision** where a plan's service area covers only part of a county (the filed partial-county ZIP lists re-rank each affected ZIP's own plan set; 2026: 18 counties in MI, OR, TX); otherwise it falls back to the built-in state-level SLCSP estimate (SBM states without a published 2026 PUF — Colorado and Maryland — use hand-maintained reference estimates) and treats that benchmark as the selected plan. In every premium path (manual age-rated, ZIP rating-area, SBE estimate, and state fallback), household members age 65+ are excluded from the marketplace premium rather than clamped to the age-64 rate, so a mixed-age household is not charged ACA premiums for a spouse the simulator is already billing Medicare Part B/D premiums for; ZIP-mode runs without entered member ages derive the covered ages from the modeled primary/spouse ages. Exact selected plan mode requires the household SLCSP monthly premium, selected plan monthly premium, selected plan OOP max, and covered member ages; subsidy math uses the SLCSP while medical spending uses the selected plan. Manual ACA premiums and ZIP-derived benchmarks are age-rated and inflated forward; selected-plan OOP maximums inflate as dollar limits, but are not age-rated.
...
Retirement-tax controls include Roth contribution basis, whether the Roth five-year qualified-distribution rule is satisfied, annual early-withdrawal penalty exception amounts, RMD start-age override, Social Security benefit timing, Medicare/IRMAA enrollment and lookback inputs plus a Medicare annual out-of-pocket estimate applied once the household is fully Medicare-eligible, earned-income inputs for W-2 employee FICA, Additional Medicare Tax, Schedule SE self-employment tax, and ACTC earned-income sizing, optional W-2 Social Security wages for wage-base coordination, spouse Medicare/SS wages and self-employment income for the opt-in spouse PIA estimate, dynamic child ages, age-65 standard-deduction bumps, the 2025-2028 enhanced senior deduction, Schedule A itemized deduction inputs (deduction choice, SALT paid, mortgage interest, charitable gifts, and medical expenses), an AMT preference/addback tripwire input, QBI deduction planning inputs (source, amount, SSTB status, W-2 wages, and UBIA property), heir ordinary tax rate for after-tax bequest estimates, and manual federal deduction/credit overrides. Itemized inputs trigger CPA-review confidence flags because the model applies the 2026 SALT cap/phaseout and medical-expense AGI floor but treats mortgage interest and charitable gifts as already deductible/substantiated amounts. ACTC-sensitive years trigger Schedule 8812 CPA-review flags because the model applies the common earned-income formula but does not validate SSNs, qualifying-child facts, EITC coordination, Puerto Rico rules, combat-pay/Medicaid-waiver choices, Form 2555, or the three-or-more-child Social Security tax comparison. AMT exposure triggers CPA-review confidence flags when the household enters Form 6251 preference/addback estimates or modeled income gets near the 2026 AMT exemption phaseout; the app does not calculate tentative minimum tax or full Form 6251 adjustments. QBI inputs trigger CPA-review confidence flags because the model applies the 2026 Section 199A planning formula but does not validate trade/business status, K-1 aggregation, loss carryforwards, REIT/PTP components, reasonable compensation, or full Form 8995-A facts. Nonzero manual federal deduction/credit overrides trigger CPA-review confidence flags because the model applies them mechanically but does not validate eligibility, phaseouts, refundability, or AMT/QBI interactions. Self-employment income triggers a CPA-review confidence flag because business/pass-through items beyond Schedule SE and optional QBI planning still need review.

After-tax bequest and legacy modeling features a progressive heir taxation engine. The simulator models spouse rollover (tax-deferred), non-spouse 10-year distributions stacked progressively on the heir's starting base income (default $80,000 Single) to simulate bracket compression, and eligible-designated stretch distributions over a life-expectancy schedule (a generic table approximating the IRS Single Life Table). Heir income tax always uses Single-filer brackets and the Single standard deduction regardless of the household's filing status, and the heir brackets, deduction, base income, and federal estate exclusion are indexed to the plan's price level at the death/valuation year. The household-level heir beneficiary type is the default, and individual portfolio accounts can override it with their own `beneficiaryType` when an IRA, HSA, Roth, or taxable account has a different beneficiary. It models the Federal Estate Tax (40% above the 2026 $15M per-decedent exclusion; surviving-spouse transfers are exempt under the unlimited marital deduction) and lineal-heir state inheritance tax (PA 4.5%, NE 1%; NJ Class A and MD lineal descendants are exempt — non-lineal heirs face higher rates that are out of model). It assumes taxable-account unrealized gains receive a basis step-up at death. Heir income/age default to $80,000/age 30 when unset and materially affect the result, so set them for your heirs.

The default withdrawal strategy keeps the app's established current-year heuristic. The optional lifetime optimizer evaluates multiple account-source orders each year, preserves Roth basis unless modeled tax/medical savings meet the configured hurdle, prefers lower expected-return lots when tax costs are otherwise similar, can use MAGI thresholds to size Roth-basis substitutions, and can harvest gains beyond the 0% long-term capital gains bracket when the modeled current-year cost is lower than an estimated future capital-gains tax rate. The essential/discretionary spending guardrail mode follows the stock-market drawdown rule popularized by Mad Fientist and Of Dollars and Data: essential spending inflates, discretionary spending stays nominal, and discretionary spending is modeled at 100%, 50%, or 0% depending on whether the prior year-end stock index is less than 10%, 10-20%, or more than 20% below its high-water mark. Risk-based historical guardrails solve spending and trigger levels from the selected historical backtest set: a fixed-spend failsafe, an initial spend at the target success rate, a lower portfolio trigger that cuts spend back to the target, and an upper portfolio trigger that raises spend while preserving the configured success target. The separate sequence-risk reserve option lets users test cash/T-bill, bond/TIPS, or hybrid reserve behavior: reserve assets are preserved in non-stress early years and spent first when stock returns are negative during the configured reserve tent window. The TIPS bond ladder option carves an N-year held-to-maturity inflation-indexed ladder out of the portfolio at plan start: rungs maturing before age 59½ are funded taxable-first (penalty-free), later rungs traditional-first (maturities count toward RMDs), each rung funds one year of real spending before any other withdrawal, and the rungs are excluded from rebalancing, asset-location swaps, reserve counting, and Roth conversions. Four maintenance policies govern the ladder after the initial carve-out: build-once-deplete (default — a declining bond tent leaving a rising equity glidepath), always-replenish, replenish-after-up-years (with optional catch-up of missed rungs), and spend-rungs-only-in-down-years (rungs roll forward at the locked real yield in up years — the policy tournament's efficiency winner for success rate per unit of heir value given up). The decision engine also tests the ladder as a rescue option (5/10/15/20-year variants, or alternate lengths when one is already enabled). Roth conversion sizing is spending-aware by default: conversions fill standard-deduction and low-bracket room on top of the year's RMDs, TIPS rung maturities, realized strategy gains, and a two-pass estimate of the spending withdrawal (including the cash to pay the conversion's own taxes), so ACA premium tax credits survive ladder and RMD years instead of being vaporized at the 400% FPL cliff. The companion basis-swap option funds ACA-year spending from spendable Roth value when it covers the gap and converts the freed MAGI headroom instead — "spend basis, convert the room" — with the withdrawal planner's threshold-targeted Roth substitution realizing the funding choice only when modeled tax-plus-premium savings clear the configured hurdle.
...
Monte Carlo defaults use the 2026 market-neutral preset: 1000 correlated annual paths, nominal before-tax/before-fee return assumptions, and arithmetic annual draw means converted from public geometric capital-market assumptions. The sampling mode can also be switched to independent annual draws or mean-reverting correlated draws; the mean-reverting mode keeps asset-class factor correlations while pulling future returns against one-year excess returns and accumulated long-term trend drift. The conservative planning and historical-fit presets remain available. Crypto stays a speculative BTC-like assumption rather than a capital-market-assumption median.
