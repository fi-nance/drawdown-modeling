...
- ACA benchmark premiums are currently state-level defaults age-rated with the federal default age curve. Exact selected-plan mode can bypass the selected-plan assumption by using the household SLCSP monthly premium, selected plan monthly premium, selected plan OOP max, and covered member ages from Healthcare.gov or a state exchange. For automated production-grade precision, replace state defaults with CMS/state PUF-derived benchmark and plan premiums by county, rating area, exact household member ages, tobacco status where relevant, and household member mix.
- Federal tax currently models the standard deduction, age-65 additional standard deduction, progressive ordinary income tax, preferential long-term capital gain and qualified dividend stacking, dynamic child tax credit counts from child ages, Social Security taxable-benefit phase-in, NIIT, capital-loss offsets and carryforwards, and manual additional deduction/credit overrides.
- Retirement cash-flow modeling can force RMDs from traditional accounts, collect Social Security as an income stream, retain unspent forced distributions as taxable cash, and estimate Medicare Part B/D IRMAA using available two-year lookback MAGI.
